...
export async function getUserStats(db: Database, userId: string): Promise<UserStats> {
	...
	const dayRows = await db
		.select({
			day: sql<string>`date(${questionAnswers.answeredAt}, 'unixepoch')`,
		})
		.from(questionAnswers)
		.innerJoin(quizAttempts, eq(quizAttempts.id, questionAnswers.attemptId))
		.where(eq(quizAttempts.userId, userId))
		.groupBy(sql`date(${questionAnswers.answeredAt}, 'unixepoch')`)
		.orderBy(desc(sql`date(${questionAnswers.answeredAt}, 'unixepoch')`));
