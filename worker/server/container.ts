import { createEmailService } from "@api/auth/email";
import { createAuthService } from "@api/auth/service";
import type { AuthStore } from "@api/auth/store";
...
export function createContainer(env: Env) {
	...
	// The user repository is the app's implementation of the auth module's
	// storage seam; this assignment is the compile-time conformance check.
	const authStore: AuthStore = repositories.users;

	const services = {
		auth: createAuthService(authStore, env.JWT_SECRET),
