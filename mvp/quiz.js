...
(() => {
	...
	function renderQuiz() {
		...
		$("progress-label").textContent = `Questão ${S().index + 1} de ${total}`;
		$("progress-fill").style.width = total ? `${((S().index + 1) / total) * 100}%` : "0%";
		...
		if (!S().corrected) {
			...
			if (unanswered > 0) {
				const ok = await Quiz.ui.confirm({
					...
					message: `Você ainda não respondeu ${unanswered} questão(ões). O resultado vai contar essas questões como erradas.`,
	...
	function renderWrongList(wrongIds) {
		...
		wrongIds.forEach((id) => {
			...
			item.innerHTML =
				...
				esc(q.statement) +
				"</div>" +
				'<div class="line your">Sua resposta: ' +
				esc(selected) +
				(selOpt ? ` — ${esc(selOpt.text)}` : "") +
				"</div>" +
				'<div class="line right">Correta: ' +
				esc(q.answer) +
				" — " +
				esc(rightOpt.text) +
				"</div>" +
	...
	async function redoWrongOnly() {
		...
		const ok = await Quiz.ui.confirm({
			...
			message:
				"Isto limpa suas respostas erradas para você responder de novo. As respostas certas serão mantidas.",
