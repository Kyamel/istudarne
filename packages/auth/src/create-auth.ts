 ...
 * session listing/revocation, a new-device sign-in alert, and dual transport
 * (httpOnly cookies + `Authorization: Bearer`). Where the old modules hand-rolled
 * JWTs, opaque refresh-token rotation and PBKDF2, Better Auth now owns all of it
 * with stateful, immediately-revocable sessions.
 *
...
export function createAuth(config: CreateAuthConfig) {
	...
	return betterAuth({
		...
		plugins: [
			// `Authorization: Bearer <token>` transport for native/mobile clients.
			bearer(),
			// Multiple concurrent device sessions per user.
			multiSession(),
			// Serves the OpenAPI schema + reference UI under /api/auth/reference.
			openAPI(),
			...(config.plugins ?? []),
		],
	});
